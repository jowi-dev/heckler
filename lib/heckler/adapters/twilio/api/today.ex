...
defmodule Heckler.Adapters.Twilio.Api.Today do
  @moduledoc """
  API calls for all endpoints tagged `Today`.
  """
  ...
  - `opts` (keyword): Optional parameters
    - `:Category` (UsageRecordTodayEnumCategory): The [usage category](https://www.twilio.com/docs/usage/api/usage-record#usage-categories) of the UsageRecord resources to read. Only UsageRecord resources in the specified category are retrieved.
  ...
  - `{:ok, Heckler.Adapters.Twilio.Model.ListUsageRecordTodayResponse.t()}` on success
  - `{:error, Tesla.Env.t()}` on failure
  """
  @spec list_usage_record_today(Tesla.Env.client, String.t, keyword()) :: {:ok, Heckler.Adapters.Twilio.Model.ListUsageRecordTodayResponse.t()} | {:error, Tesla.Env.t()}
  def list_usage_record_today(connection, account_sid, opts \\ []) do
    optional_params = %{
      :Category => :query,
      :StartDate => :query,
      :EndDate => :query,
    ...
    request =
      %{}
      |> method(:get)
      |> url("/2010-04-01/Accounts/#{account_sid}/Usage/Records/Today.json")
    ...
    |> evaluate_response([
      {200, Heckler.Adapters.Twilio.Model.ListUsageRecordTodayResponse}
    ])
  end
end
