...
defmodule Heckler.Adapters.Twilio.Api.AllTime do
  @moduledoc """
  API calls for all endpoints tagged `AllTime`.
  """
  ...
  - `opts` (keyword): Optional parameters
    - `:Category` (UsageRecordAllTimeEnumCategory): The [usage category](https://www.twilio.com/docs/usage/api/usage-record#usage-categories) of the UsageRecord resources to read. Only UsageRecord resources in the specified category are retrieved.
  ...
  - `{:ok, Heckler.Adapters.Twilio.Model.ListUsageRecordAllTimeResponse.t()}` on success
  - `{:error, Tesla.Env.t()}` on failure
  """
  @spec list_usage_record_all_time(Tesla.Env.client, String.t, keyword()) :: {:ok, Heckler.Adapters.Twilio.Model.ListUsageRecordAllTimeResponse.t()} | {:error, Tesla.Env.t()}
  def list_usage_record_all_time(connection, account_sid, opts \\ []) do
    optional_params = %{
      :Category => :query,
      :StartDate => :query,
      :EndDate => :query,
    ...
    request =
      %{}
      |> method(:get)
      |> url("/2010-04-01/Accounts/#{account_sid}/Usage/Records/AllTime.json")
    ...
    |> evaluate_response([
      {200, Heckler.Adapters.Twilio.Model.ListUsageRecordAllTimeResponse}
    ])
  end
end
